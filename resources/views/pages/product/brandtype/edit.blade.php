...
@section('content')
    <div class="page-header">
        <div class="page-title">
            <h4>Brand Type Edit</h4>
            <h6>Edit Brand Type</h6>
        </div>
    </div>
    <div class="card">
        <div class="card-body">
            <div class="row">
                <div class="col-lg-12 col-sm-6 col-12">
                    <form action="{{ route('brandtype.update', $item->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="form-group">
                            <label>Brand Type Name</label>
                            <div class="input-groupicon">
                                <input type="text" name="name" placeholder="Brand Type Name"
                                    value="{{ $item->name }}" />
                            </div>
                        </div>
                        <div class="form-group">
                            <label>Slug</label>
                            <div class="input-groupicon">
                                <input type="text" name="slug" placeholder="Brand Type Slug"
                                    value="{{ $item->slug }}" />
                            </div>
                        </div>
                        <div class="form-group">
                            <button type="submit" class="btn btn-submit me-2">Submit</button>
                            <a href="{{ route('brandtype.index') }}" class="btn btn-cancel">Cancel</a>
                        </div>
                    </form>
                </div>
            </div>
